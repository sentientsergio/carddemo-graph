...
- **Q5 — Cluster by shared data access** (informational, not gold-asserted). Returns candidate bounded-context clusters with straddle programs flagged. See `clustering_report.md`.

Cypher implementations live in `src/carddemo_graph/pilot.py` (Gate 2 demonstration) and `src/carddemo_graph/gate3.py` (full corpus). Standalone `.cypher` files are not separately committed in v1.
...
See `queries/README.md` for additional patterns and `src/carddemo_graph/pilot.py` / `gate3.py` for the Q1-Q4 implementations.
